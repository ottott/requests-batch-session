# Requests + BatchSession

This repository is a personal fork of the Requests library that adds support for concurrent batch execution through a new `BatchSession` class.

## Features

- Queue multiple HTTP requests before execution.
- Execute queued requests concurrently using `ThreadPoolExecutor`.
- Preserve request order in the returned results.
- Return a `BatchResult` for each request, allowing individual failures without stopping the batch.
...
batch = BatchSession(workers=8)

batch.queue_get("https://example.com")
batch.queue_get("https://httpbin.org/get")
...
50 requests to `https://httpbin.org/delay/1`

| Method | Time |
|--------|------:|
| Sequential `Session` | 6.33 s |
| `BatchSession` | 1.65 s |
